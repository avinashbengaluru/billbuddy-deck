...
deck_script = """
    (function () {
      ...
      window.addEventListener('touchend', function (e) {
        if (!e.changedTouches || e.changedTouches.length !== 1 || morphing) return;
        const dx = e.changedTouches[0].clientX - touchStartX;
        const dy = e.changedTouches[0].clientY - touchStartY;
        if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 40) {
          if (dx < 0 && index < slides.length - 1) goTo(index + 1);
...
{dots_html}
    ...
    <a href="mobile.html" class="nav-toggle-btn" title="Switch to Mobile View" style="font-size: 0.65rem; font-weight: 700; color: #5eead4; background: rgba(45,212,191,0.14); border: 1px solid rgba(45,212,191,0.35); padding: 0.25rem 0.55rem; border-radius: 999px; text-decoration: none; display: flex; align-items: center; gap: 0.25rem; margin-left: 0.2rem;">📱 Mobile</a>
  </nav>
